...
SELECT * FROM (
    ...
    SELECT
        ...
        NULL AS `明细银行账号`
    FROM formtable_main_83 m
    JOIN formtable_main_83_dt1 d ON d.mainid = m.id
    LEFT JOIN workflow_requestbase rb ON rb.REQUESTID = m.requestid
    LEFT JOIN uf_cbzx cc ON cc.id = m.cbzx
    WHERE NOT EXISTS (
        SELECT 1 FROM formtable_main_83_dt3 x WHERE x.mainid = m.id
    )
      AND NOT EXISTS (
        SELECT 1 FROM formtable_main_83_dt4 x WHERE x.mainid = m.id
    )
      AND NOT EXISTS (
        SELECT 1 FROM formtable_main_83_dt5 x WHERE x.mainid = m.id
    )
      AND NOT EXISTS (
        SELECT 1 FROM formtable_main_83_dt6 x WHERE x.mainid = m.id
    )
) x
WHERE (
...
MCN_ANCHOR_SOURCE_SELECTS = """
    SELECT
        ...
        m.szht AS `主表合同ID`,
        {contract_expr} AS `明细合同ID`,
        d.xmbh AS `项目编号ID`,
        d.xmmc AS `项目名称`,
        {fee_code_expr} AS `费用项编码`,
        {fee_name_expr} AS `费用项名称`,
        {amount_expr} AS `金额`,
        d.zbid AS `主播房间号`,
        d.zbnc AS `主播昵称`,
        d.sfzh AS `身份证号`,
...
MCN_ANCHOR_DETAIL_SELECTS = [
    MCN_ANCHOR_SOURCE_SELECTS.format(
        detail_table='formtable_main_38_dt4',
        contract_expr='d.xght',
        fee_code_expr='d.fjxh',
        fee_name_expr='d.fyx',
        amount_expr='COALESCE(d.dkje, d.yfuje, d.skje, d.yfaje)',
        detail_filter='',
    ),
    MCN_ANCHOR_SOURCE_SELECTS.format(
        detail_table='formtable_main_38_dt2',
        contract_expr="''",
        fee_code_expr="''",
        fee_name_expr="''",
        amount_expr='COALESCE(d.dkje, d.yfje, d.skje)',
        detail_filter="""
    WHERE NOT EXISTS (
    ...
    MCN_ANCHOR_SOURCE_SELECTS.format(
        detail_table='formtable_main_38_dt1',
        contract_expr="''",
        fee_code_expr="''",
        fee_name_expr="''",
        amount_expr='COALESCE(d.dkje, d.yfje, d.skje)',
    ...
    MCN_ANCHOR_SOURCE_SELECTS.format(
        detail_table='formtable_main_38_dt3',
        contract_expr="''",
        fee_code_expr="''",
        fee_name_expr="''",
        amount_expr='COALESCE(d.dkje, d.yfje, d.skje)',
    ...
    WHERE NOT EXISTS (
      ...
      AND NOT EXISTS (
        SELECT 1 FROM formtable_main_38_dt1 x WHERE x.mainid = m.id
    )""",
    ),
]
...
FROM (
    SELECT m.yfklc AS `关联流程ID`, d.szxm AS `项目编号ID`, d.fjxh AS `费用项编码`, ABS(COALESCE(d.hsje, 0)) AS `金额`
    FROM formtable_main_82 m JOIN formtable_main_82_dt1 d ON d.mainid = m.id
    WHERE NOT EXISTS (
        SELECT 1 FROM formtable_main_82_dt2 x WHERE x.mainid = m.id
    )
      AND NOT EXISTS (
        SELECT 1 FROM formtable_main_82_dt3 x WHERE x.mainid = m.id
    )
      AND NOT EXISTS (
        SELECT 1 FROM formtable_main_82_dt4 x WHERE x.mainid = m.id
    )
      AND NOT EXISTS (
        SELECT 1 FROM formtable_main_82_dt6 x WHERE x.mainid = m.id
    )
    UNION ALL
...
def _apply_mcn_anchor_fee_usage(source_df):
    """MCN 主播 dt1/dt2/dt3 兜底行没有费用项字段,按页面费用使用视图回填。"""
    ...
    if filled:
        print(f'[预付期初-MCN主播相关付款流程] 项目费用使用情况回填费用项 {filled} 行')
    return result
