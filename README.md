...
每个任务除导入文件外，还会生成 `未匹配清单_<业务>_<YYYYMMDD>.xlsx`，按需生成以下 sheet：

- **必输字段未达100%**：模板里有底色的必输字段，凡填充率 < 100% 的都列在此（字段、缺失数、填充率）。
- **未匹配_工号 / 未匹配_供应商 / 未匹配_核算主体 / 未匹配_费用科目**：对应映射没匹配上的源名称清单，供补主数据。**某类全部匹配上时，该 sheet 不生成。**
